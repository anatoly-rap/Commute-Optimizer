...
def fig_to_HTML(fig): return fig.to_html(include_plotlyjs='cdn')

def calculate_metrics(data): return data['SpeedMPH'].min(), data['SpeedMPH'].max(), data['SpeedMPH'].mean()
...
def setup_header_label():
    ...
    header_label.setSizePolicy(QSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed))
    return header_label
...
def setup_speed_view(data):
    ...
    speed_view.setHtml(gen_speed_time_graph(data, min_speed, max_speed, avg_speed).to_html(include_plotlyjs='cdn'))
    return speed_view
...
def main():
    ...
    grid_layout.addWidget(setup_header_label(), 0, 0, 1, 2)
    grid_layout.addWidget(setup_map_view(load_trip()), 1, 0)
    grid_layout.addWidget(setup_speed_view(load_trip()), 1, 1) 
